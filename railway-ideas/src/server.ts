...
const IDEATION_MODEL = process.env.OPENAI_MODEL || "gpt-4o-mini"; // Allow model override
...
const MAX_CLUSTERS_PER_BATCH = 5; // Process clusters in smaller batches
...
function clusterPostsBySimilarity(
  ...
  for (let i = 0; i < postsWithEmbeddings.length; i++) {
    ...
    // IMPROVED CLUSTERING: Use iterative expansion to find all related posts
    let foundNew = true;
    while (foundNew) {
      foundNew = false;
      ...
      for (let j = 0; j < postsWithEmbeddings.length; j++) {
        ...
        // Check similarity to ANY post in the current cluster, not just seed
        let maxSimilarity = 0;
        let bestMatch = null;
        
        for (const clusterPost of clusterPosts) {
          const similarity = cosineSimilarity(
            clusterPost.embedding,
        ...
        if (maxSimilarity >= similarityThreshold) {
          console.log(
            `Found similar post ${candidatePost.id} with similarity ${maxSimilarity.toFixed(4)} to post ${bestMatch?.id}`
          );
          clusterPosts.push(candidatePost);
          processed.add(j);
          foundNew = true; // Continue searching as new post might attract others
        }
      }
    }
    ...
    if (clusterPosts.length >= minClusterSize) {
      ...
      // NO LIMIT - Use ALL posts as representative posts
  ...
  // Sort clusters by size (largest first) - NO LIMIT on number of clusters
  ...
  return clusters; // Return ALL clusters, no slicing
}
...
async function generateClusterTheme(cluster: PostCluster): Promise<string> {
  ...
  // Use MORE posts for better theme generation - up to 10 instead of 5
...
function buildEnhancedPrompt(cluster: PostCluster, existingIdeas: string[]): { system: string; user: string } {
...
Return STRICT JSON exactly in this shape:
{
  ...
  // Use ALL representative posts, not just 5 - NO LIMITS!
  const postTexts = cluster.representative_posts.map((post: any) => {
    const content = `${post.title || ""}\n\n${post.body || ""}`.replace(/\s+/g, " ").trim();
    const truncated = content.slice(0, 500); // More content for better context
...
async function callOpenAI(system: string, user: string, retryWithFallback = true): Promise<any> {
  if (!OPENAI_API_KEY) {
        ...
        if (!response.ok) {
          ...
          if (response.status === 429) {
            ...
            try {
              ...
              if (waitMatch) {
                waitTime = Math.max(parseFloat(waitMatch[1]) * 1000 + 5000, waitTime); // Add 5s buffer
              }
  ...
  if (clusters.length === 0) {
    await updateJobStatus(jobId, {
      ...
      result: {
        ...
        } and min size ${parameters.min_cluster_size || 3}`,
        ideas_generated: 0,
      },
    });
    return { ideas: [], clusters_processed: 0, posts_processed: rawPosts.length };
...
async function storeIdeas(ideas: any[], parameters: any): Promise<{ run_id: number, inserted_count: number }> {
  // Create run header
