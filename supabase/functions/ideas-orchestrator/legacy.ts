...
async function getJob(jobId) {
  ...
  try {
    const { data, error } = await supabase.from("ideas_jobs").select("*").eq("id", jobId).single();
    if (error || !data) {
      return null;
    }
    return {
...
function buildEnhancedPrompt(items, existingIdeas) {
...
2. Generate 3-7 DIVERSE SaaS ideas; ensure at least 3 are meaningfully different in target users, industries, or workflows.
3. Each idea must address similar pain points mentioned across DIFFERENT posts.
4. Focus on solutions that can serve many customers with the same underlying problem.
5. Prioritize B2B SaaS opportunities with clear willingness to pay and revenue potential.
6. Perform a lightweight existence check: if similar products exist, explain how this idea is differentiated; if unsure, set "does_not_exist" to "unknown".
7. Ensure ideas are technically feasible with today's SaaS stack (APIs, webhooks, ETL/ELT, LLMs, cloud infra) and avoid vague "AI does everything" claims.
...
async function generateIdeasWithReliability(summaryLines, existingIdeas) {
  if (!OPENAI_API_KEY) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          if (attempt < MAX_RETRIES) {
            const delay = Math.min(EXPONENTIAL_BACKOFF_BASE * Math.pow(2, attempt - 1), MAX_EXPONENTIAL_DELAY);
...
function deduplicateIdeas(newIdeas, existingIdeas, threshold = 0.8) {
  const allExisting = [
    ...existingIdeas
  ];
  const result = [];
...
async function executeIdeasJob(jobId) {
  ...
  try {
    ...
    const { data: posts, error: postsError } = await supabase.from("posts").select("id, summary, sentiment, url, created_at, platform").eq("platform", platform).eq("is_complaint", true).lt("sentiment", -0.1).not("summary", "is", null).gte("created_at", sinceISO).order("sentiment", {
      ascending: true
    }).order("created_at", {
      ascending: false
    ...
    const { data: existingIdeas } = await supabase.from("saas_idea_items").select("name, target_user, name_norm").gte("created_at", dedupeSinceISO);
    const existingIdeaNames = (existingIdeas || []).map((idea)=>`${idea.name} (Target: ${idea.target_user || "N/A"})`);
    // Create run header
    const { data: runData, error: runError } = await supabase.from("saas_idea_runs").insert({
    ...
    const preparedIdeas = allIdeas.map((idea)=>{
      const postIds = Array.isArray(idea.representative_post_ids) ? idea.representative_post_ids.filter((id)=>Number.isInteger(Number(id))).map(Number) : [];
      return {
        run_id: runId,
        ...
        posts_in_common: postIds.length,
    ...
    if (preparedIdeas.length > 0) {
      const { data: insertedIdeas, error: insertError } = await supabase.from("saas_idea_items").upsert(preparedIdeas, {
